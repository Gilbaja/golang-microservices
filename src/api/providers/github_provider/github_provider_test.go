...
func TestCreateRepoInvalidSuccessResponse(t *testing.T) {
	...
	assert.NotNil(t, err)
	assert.EqualValues(t, http.StatusInternalServerError, err.StatusCode)
	assert.EqualValues(t, "error when trying to unmarshal create repo response", err.Message)
}
...
func TestCreateRepoNoError(t *testing.T) {
	restclient.AddMockup(restclient.Mock{
		...
		Response: &http.Response{
			StatusCode: http.StatusCreated,
			Body:       ioutil.NopCloser(strings.NewReader(`{"id": 123,"name": "golang-tutorial","full_name": "Gilbaja/golang-tutorial"}`)),
		},
	})
	...
	assert.EqualValues(t, "Gilbaja/golang-tutorial", response.FullName)
}
